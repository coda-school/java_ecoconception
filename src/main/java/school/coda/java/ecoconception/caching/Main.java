...
import static school.coda.java.ecoconception.caching.Caching.compute_ad_hoc_Float;
import static school.coda.java.ecoconception.caching.Caching.compute_ad_hoc_double;
import static school.coda.java.ecoconception.caching.Caching.compute_ad_hoc_float;
import static school.coda.java.ecoconception.caching.Caching.compute_ad_hoc_float_cast_at_end;
import static school.coda.java.ecoconception.caching.Caching.compute_factored_cached_Double;
import static school.coda.java.ecoconception.caching.Caching.compute_factored_cached_double;
import static school.coda.java.ecoconception.caching.Caching.compute_factored_cached_float;
...
public class Main {
    ...
    static void main() {
        ...
        {
            System.out.println("Iterations : compute_ad_hoc_float");
            LocalDateTime start = LocalDateTime.now();
            for (int i = 0; i < iterations; i++) {
                for (double angle : angles) {
                    compute_ad_hoc_float((float) angle);
                }
            }
        ...
        {
            System.out.println("Iterations : compute_ad_hoc_float_cast_at_end");
            LocalDateTime start = LocalDateTime.now();
            for (int i = 0; i < iterations; i++) {
                for (double angle : angles)
                    compute_ad_hoc_float_cast_at_end(angle);
        ...
        {
            System.out.println("Iterations : compute_factored_cached_float");
            LocalDateTime start = LocalDateTime.now();
            for (int i = 0; i < iterations; i++) {
                for (double angle : angles)
                    compute_factored_cached_float((float) angle);
        ...
        {
            System.out.println("Iterations : compute_ad_hoc_double");
            LocalDateTime start = LocalDateTime.now();
            for (int i = 0; i < iterations; i++) {
                for (double angle : angles)
                    compute_ad_hoc_double(angle);
        ...
        {
            System.out.println("Iterations : compute_factored_cached_double");
            LocalDateTime start = LocalDateTime.now();
            for (int i = 0; i < iterations; i++) {
                for (double angle : angles)
                    compute_factored_cached_double(angle);
        ...
        {
            System.out.println("Stream : compute_factored_cached_double");
            LocalDateTime start = LocalDateTime.now();
            IntStream.range(0, iterations)
                    .forEach(_ ->
                            Arrays.stream(angles)
                                    .forEach(Caching::compute_factored_cached_double));
            LocalDateTime end = LocalDateTime.now();
        ...
        {
            System.out.println("Stream parallel: compute_factored_cached_double");
            LocalDateTime start = LocalDateTime.now();
            IntStream.range(0, iterations)
                    .forEach(_ ->
                            Arrays.stream(angles).parallel()
                                    .forEach(Caching::compute_factored_cached_double));
            LocalDateTime end = LocalDateTime.now();
        ...
        {
            System.out.println("Iteration: compute_ad_hoc_Float - type boxés");
            LocalDateTime start = LocalDateTime.now();
            // Avec des types boxés
            // Il y a des allocations dynamiques de mémoire
            for (int i = 0; i < iterations; i++) {
                for (double angle : angles)
                    compute_ad_hoc_Float(angle);
        ...
        {
            System.out.println("Iteration: compute_factored_cached_Double - type boxés");
            LocalDateTime start = LocalDateTime.now();
            for (int i = 0; i < iterations; i++) {
                for (double angle : angles)
                    compute_factored_cached_Double(angle);
